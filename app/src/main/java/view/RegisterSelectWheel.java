...
public class RegisterSelectWheel extends BaseWheelView implements View.OnClickListener, OnWheelChangedListener {
    ...
    private void setUpViews() {
        mViewSchool = findViewById(R.id.id_province);
    ...
    private void setUpData() {
        initSchoolDatas();
        mViewSchool.setViewAdapter(new ArrayWheelAdapter<String>(RegisterSelectWheel.this, mSchoolDatas));
        // 设置可见条目数量
    ...
    private void updateClass() {
        ...
        mCurrentDepartName = mDepartDatasMap.get(mCurrentSchoolName)[pCurrent];
    ...
    private void updateDepart() {
        int pCurrent = mViewSchool.getCurrentItem();
        mCurrentSchoolName = mSchoolDatas[pCurrent];
        String[] departs = mDepartDatasMap.get(mCurrentSchoolName);
        if (departs == null) {
            departs = new String[] { "" };
        }
    ...
    private void showSelectedResult() {
        ...
        // 获取用户计算后的结果
        intent.putExtra("school", mCurrentSchoolName); //将计算的值回传回去
        intent.putExtra("depart", mDepartDatasMap.get(mCurrentSchoolName)[mViewDepart.getCurrentItem()]);
